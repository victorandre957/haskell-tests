...
execute context x = case x of
  ...
  SWhile exp stm -> execute context (SBlock [stm, SdoWhile stm exp])
  SdoWhile stm exp -> if (eval context exp) /= 0 then execute (execute context stm) (SdoWhile stm exp) else context
